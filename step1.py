...
def all_to_csv():
    class Vendor:
        ...
        def change_words(self, df):
            ...
            if self.name == 'Slami':
                # Этот блок нужен для Roland. Оставляем поле Уточняйте, что бы получить наличие.
                exclude_values = ['не для продажи в РФ', 'Çâîíèòå', '0', 'Звоните', 'витрина']
                # Переименовываем ОПТ на РРЦ, и у нас оба столбца показывают РРЦ
                df.loc[df['ОПТ'] == 'Уточняйте', 'ОПТ'] = df['РРЦ']
